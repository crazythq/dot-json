...
enum JSONParser {
    ...
    private static func serializeAny(_ value: Any) -> String {
        // JSONSerialization 只接受容器（dict/array），标量必须包装。
        let container = (value is NSDictionary || value is NSArray) ? value : [value]
        guard let data = try? JSONSerialization.data(
            withJSONObject: container,
            options: [.sortedKeys, .withoutEscapingSlashes]
        ) else { return "null" }
        var result = String(data: data, encoding: .utf8) ?? "null"
        // 去除包装数组的 "[" 和 "]"
        if !(value is NSDictionary || value is NSArray), result.hasPrefix("["), result.hasSuffix("]") {
