...
enum CLIApplication {
    ...
    ///
    /// - Parameters:
    ///   - arguments: 不包含可执行文件名的参数。
    ///   - environment: stdin、文件系统等外部能力。
    /// - Returns: 不会部分写出的完整 CLI 结果。
    ...
    private static func execute(
        ...
        case .format(let inputPath, let indent, let destination):
            ...
            let output: String
            do {
                output = try JSONFormatter.format(input, indent: indent)
            } catch {
                throw invalidJSON(from: error, input: input)
            }
        ...
        case .minify(let inputPath, let destination):
            ...
            let output: String
            do {
                output = try JSONFormatter.minify(input)
            } catch {
                throw invalidJSON(from: error, input: input)
            }
            return try deliver(output, inputPath: inputPath, destination: destination, environment: environment)
        case .repair(let inputPath, let destination):
            ...
            do {
                ...
                let location = location(in: input, byteOffset: error.byteOffset)
    ...
    private static func validate(
        ...
        do {
            ...
            case .json:
                ...
                return CLIResult(
                    stdout: encodeJSONObject(["valid": false, "error": detail]),
                    stderr: "",
                    exitCode: 4
                )
            case .human:
